...
validateMethodM3 <- function(method=NULL){
  ...
  if(!all(c("x","rank")%in%names(formals(method)))){
    stop("method must have input parameters 'x' and 'rank'")
  }
  ...
  # check if rank has a default value, if so, issue a warning to the user
  if(!is.null(f[["rank"]])){
    warning("'rank' should not have a default value. The default provided will be disregarded when invoking the method")
  }
  ...
  # check that all parameters other than x and rank have defaults, if not, issue error
  cond <- f[-which(names(f)%in%c("x","rank"))]
  if(length(cond)>0){
    if(any(unlist(lapply(cond,function(x) is.null(x))))){
      stop("All parameters other than 'x' and 'rank' in 'method' must have default values")
    }    
  }
