...
validateMethodM1 <- function(method=NULL){
  ...
  # check that all parameters other than x and data have defaults, if not, issue error
  cond <- f[-which(names(f)%in%c("x","data"))]
  if(length(cond)>0){
    if(any(unlist(lapply(cond,function(x) is.null(x))))){
      stop("All parameters other than 'x' and 'data' in 'method' must have default values")
    }
  }
}
