...
test_that("an error is thrown if method is NULL",{
  ...
  expect_error(validateMethodM2(),msg)
  expect_error(validateMethodM2(method=NULL),msg)
})

# test_that an error is thrown if method is not a function
test_that("an error is thrown if method is not a function",{
  msg <- "method must be a function"
  expect_error(validateMethodM2(method=NA),msg)
  expect_error(validateMethodM2(method="abc"),msg)
  expect_error(validateMethodM2(method=123),msg)
  expect_error(validateMethodM2(method=as.factor("abc")),msg)
  expect_error(validateMethodM2(method=list()),msg)
  expect_error(validateMethodM2(method=data.frame()),msg)
})
...
test_that("an error is thrown if method does not have input parameters x and y",{
  ...
  expect_error(validateMethodM2(method=method),msg)
...
test_that("a warning is issued if x is provided with a default value",{
  ...
  expect_warning(validateMethodM2(method=method),msg)
...
test_that("the function executes silently if the input is correctly specified",{
  ...
  expect_silent(validateMethodM2(method=method))
